...
times_real = [10.85,4.76,3.34,5.47,7.83,6.28,13.62,9.72,7,11.78,8.95,12.66,14.53,16.72,15.41,12.25,11.34,10.47,13.22,16.59,18.14,6.85,9.57,5.57,4.49,7.94,8.87,11,14.33,3.65]
for p_index in p_indexs:
    ...
    a = m_w * m_h
    density = Np / a
    f = open(filename + "_" + str(vmax) + "_" + str(dt) + "_" + str(density) + "_" + str(dmin) + "_" + str(p_index) + ".txt", "w")
    ...
    for i in tqdm(range(0, total)):
        ...
        #calcolo la velocità degli individui
        dist_single_unit = {}
        i = 1
    ...
    plt.title('Evacuazioni', fontsize = 15)
    plt.plot(id, tu, '-', linewidth = 1)
    plt.ylabel('Tempo trascorso', fontsize = 15)
    plt.xlabel('Persona', fontsize = 15)
    plt.xticks(np.arange(0, Np_init + 1, 1)) 
    plt.yticks(np.arange(min(tu), max(tu) + 1)) 
    plt.grid(False)
    plt.tight_layout()

    plt.figure(figsize=(9,6), dpi=150)
    plt.title('Tempo evacuazione/distanza dalla porta', fontsize = 15)
    for i in range(0, len(doors_center)):
        plt.plot(sorted([people_door_dist[k][i][0] for k in people_door_dist.keys()]), tu, '.-', linewidth = 1)
    plt.ylabel('Tempo trascorso', fontsize = 15)
    plt.xlabel('Distanza dalla porta', fontsize = 15)
    plt.xticks(np.arange(0, sqrt(dom.xmax**2 + dom.ymax**2), 0.5))
    plt.yticks(np.arange(min(tu), max(tu) + 1)) 
    plt.grid(False)
    plt.tight_layout()

    plt.figure(figsize=(10,5), dpi=150)
    plt.title('Distanza da uscita ad ogni dt', fontsize = 15)
    #for p in people_ids:
    #    i = p+1
    #    plt.plot(ts[p], people_door_dist[p], '.-', linewidth = 1, label = "Persona %i" %i)
    for i in range(0, len(doors_center)):
        plt.plot(ts[0], people_door_dist[0][i], '-', linewidth = 1)
    plt.xlabel('Tempo trascorso', fontsize = 15)
    plt.ylabel('Distanza', fontsize = 15)
    plt.yticks(np.arange(0, sqrt(dom.xmax**2 + dom.ymax**2), 0.5)) 
    ...
    #for p in people_ids:
    #    i = p+1
    #    plt.plot(ts[p], people_velocities[p], '.-', linewidth = 1, label = "Persona %i" %i)
    #plot velocità asse x
    plt.subplot(2, 1, 1)
    plt.plot(ts[0], [c[0] for c in people_pos[0]], '-', linewidth = 1)
    plt.xlabel('Tempo trascorso (s)')
    plt.ylabel('Posizione individual asse x')
    plt.title('Velocità asse x ad ogni dt', fontsize = 8)
    plt.grid(False)
    #plot velocità asse y
    plt.subplot(2, 1, 2)
    plt.plot(ts[0], [c[1] for c in people_pos[0]], '-', linewidth = 1)
    plt.xlabel('Tempo trascorso (s)')
    plt.ylabel('Posizione individual asse y')
    plt.title('Velocità asse y ad ogni dt', fontsize = 8)
    plt.grid(False)

    plt.figure(figsize=(10,5), dpi=150)
    plt.title('Accelerazione ad ogni dt', fontsize = 15)
    #for p in people_ids:
    #    i = p+1
    #    plt.plot(ts[p], velocities[p], '-', linewidth = 1, label = "Persona %i" %i)
    plt.plot(ts[0], people_velocities[0], '-', linewidth = 1)
    plt.xlabel('Tempo trascorso', fontsize = 15)
    plt.ylabel('Velocità', fontsize = 15)
    plt.yticks(np.arange(0, max([max(v) for v in people_velocities.values()]), 0.1)) 
