...
def plot_people_according_to_current_door_distance(ifig, people, domain,
    ...
    ax1.imshow(domain.door_distance*(people==1),interpolation='nearest',
               extent=[domain.xmin,domain.xmax,domain.ymin,domain.ymax],
               origin='lower',alpha = 1.0,cmap='Blues')
    if (axis):
...
def sequential_update(people, people_ij, weight, shuffle = None,
                      randomstate = None, idProf = None, movD = None, fixedOrder = None):
    """
    ...
    for id in order:
        ...
        if idProf != None and id == idProf and sp.sum(people) != 1:		#professore deve aspettare che tutti escano
            #print(str(i) + " " + str(j))
            if j>19:#i <= 22 and j > 18 or i == 22 and j>19:    
                continue
        #print("--------- sequential_update : id = ",id," i = ",i," j = ",j)
        if ((i!=-1) and (j!=-1)):	#check per vedere se già evacuati
            I = [i]; J = [j]
            if (i>=1):
                im = i-1			#provo a spostare verso sinistra
                if ( (people[im,j] is not sp.ma.masked) and (people[im,j] !=1 ) ):
                    I.append(im); J.append(j)
            if (i<=height-2):
                ip = i+1			#provo a spostare verso destra
            ...
            if movD:
			#test movimento in diagonale
			# 1 2 3
			# 4 X 6
			# 7 8 9
                if (i<=height-2 and j>=1):
                    ip = i+1			#provo a spostare verso direzione 9
                    jm = j-1
                    if ( (people[ip,jm] is not sp.ma.masked) and (people[ip,jm] !=1 ) ):
                        I.append(ip); J.append(jm)
				
                if (i>=1 and j>=1):
                    im = i-1			#provo a spostare verso direzione 7
                    jm = j-1
                    if ( (people[im,jm] is not sp.ma.masked) and (people[im,jm] !=1 ) ):
                        I.append(im); J.append(jm)
					
                if (i<=height-2 and j<=width-2):
                    ip = i+1			#provo a spostare verso diagonale direzione 3
                    jp = j+1
                    if ( (people[ip,jp] is not sp.ma.masked) and (people[ip,jp] !=1 ) ):
                        I.append(ip); J.append(jp)
			
                if (j<=width-2 and i>=1):
                    jp = j+1			#provo a spostare verso diagonale direzione 1
                    im = i-1		
                    if ( (people[im,jp] is not sp.ma.masked) and (people[im,jp] !=1 ) ):
                        I.append(im); J.append(jp)
...
def parallel_update(people, people_ij, weight, friction = 0, randomstate = None, idProf = None, movD = None):
    """
    ...
    if (randomstate == None):
        randomstate = sp.random.RandomState()
    new_ij = -sp.ones((Np,3)) ## new_i new_j proba(w)
    for id in sp.arange(Np):
        i = people_ij[id,0]; j = people_ij[id,1]
        if idProf != None and id == idProf and sp.sum(people) != 1:		#professore deve aspettare che tutti escano
            if j > 19:
        ...
        if ((i!=-1) and (j!=-1)):
            ...
            new_ij[id,1] = J[pos[0]]
            new_ij[id,2] = w[pos[0]]
    ind = sp.where(new_ij[:,0]!=-1)[0]		#guarda e prende indirizzi delle persone che non sono ancora state evacuate(i == -1 guarda tutte le i degli automi) 
	
    # Find the conflicts
    import numpy as np							 #solo i e j vengono passate, axis = 0 sigifica che non restituisce una lista flatted ma una come quella data in input
...
def exit(domain, people, people_ij,exitPeople, nExit):
    #print("già usciti ",exitPeople)
    """
    ...
    if (dind.shape[0]>0):
        ...
        exits = []
        for k in people_ij[ind_in[dind]]:
            if nExit == 1:
                exits.append(1)
            else:
                i = k[0]
                j = k[1]
                if i>= 10 and (j >= 1 and j <=4):   #check uscita 1
                    #print("exit 1")
                    exits.append(1)
                else:
                    if i>= 10 and (j >= 23 and j <=25):   #check uscita 2
                        #print("exit 2")
                        exits.append(2)					
                    else:
                        print("ERRORE!!!!!")		

        people_ij[ind_in[dind],:] = [-1, -1]			#evacuati
        k = 0
		#verifica chi è stato evacuato e in che porta
        for i in range(0,len(people_ij)):
            if i not in exitPeople:
                if people_ij[i][0] == -1 and people_ij[i][1] == -1:
                    #print(people_ij)
                    #print(k)
                    #print(exits)
                    newExitPeople.append([i, exits[k]])
                    
                    #print("evacuato " + str(i) + " uscita " + str(exits[k]))
                    k = k +1
    #print("nuovi usciti", newExitPeople)	
    return people, people_ij, newExitPeople
