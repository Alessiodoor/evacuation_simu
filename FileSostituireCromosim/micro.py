...
def move_people(time, dt, people, U, crosslines=[]):
    ...
    #if(people.shape[0]>1):
    #    for i in range(0, people.shape[0]-1):
    #        people[i,0] += dt*U[i,0]
    #        people[i,1] += dt*U[i,1]
    #else:
    #    people[:,0] += dt*U[:,0]
...
def exit_door(sexit, dom, people, U, doors_center, arrays=[]):
    """
    ...
    for i in range(0, len(D)):
        if(D[i]<=sexit or type(D[i]) is np.ma.core.MaskedConstant):
            #print("Evacuato: ", str(int(people[i,3])+1))
            #id_exit.append(int(people[i,3])+1)
            x2 = people[i, 0]
            y2 = people[i, 1]
            for j in range(0, len(doors_center)):
                x1 = doors_center[j][0]
                y1 = doors_center[j][1]
            ...
            id_exit[int(people[i,3])+1] = min_distance_id

    ind = sp.where(D>sexit)
    if (len(arrays)>0):
        return people[ind[0],:], U[ind[0],:], [ a[ind[0]] for a in arrays], id_exit
    else:
        return people[ind[0],:], U[ind[0],:], id_exit
...
def people_initialization(N, init_people_box, dom, dt, rmin, rmax, dmin=0,
    ...
    ## overlaps can occur...
    people = sp.zeros((N.sum(),4))  # x y r id
    ...
    people[:,3] = sp.arange(0, people.shape[0])
...
def plot_people(ifig, dom, people, contacts, U, colors, time=-1, axis=None,
    ...
    norm = matplotlib.colors.BoundaryNorm(boundaries=np.append(colors,19), ncolors=len(colors))'''
    fig = plt.figure(ifig)
    plt.clf()
    ...
    fig.canvas.draw()
    if (savefig):
        fig.savefig(filename,dpi=600,bbox_inches='tight',pad_inches=0, facecolor=fig.get_facecolor())
		
def plot_grafici(ifig, people, ts, people_door_dist, t, velocities, people_pos):
    fig = plt.figure(ifig, figsize=(6,5), dpi=150)
    plt.clf()
    plt.subplot(2, 1, 1)
    plt.title('Distanza da uscita ad ogni dt', fontsize = 15)
    plt.xlabel('Tempo trascorso', fontsize = 10)
    plt.ylabel('Distanza', fontsize = 10)
    plt.xticks(np.arange(0, t + 1)) 
    plt.grid(False)
    plt.tight_layout()
    #if(type(people_door_dist[0][0]) is list):
    for i in range(0, len(people_door_dist[0])):
        plt.plot(ts[0], people_door_dist[0][i], '-', linewidth = 1, label = "porta %i" %i)      
        plt.yticks(np.arange(0, max([max(p) for p in people_door_dist[0]]) + 1, 1))
        plt.legend(prop={'size': 6})
    '''else:
        plt.plot(ts[0], people_door_dist[0], '-', linewidth = 1)
        plt.yticks(np.arange(0, people_door_dist[0][0] + 1, 1)) 
    ...
    #plt.plot(ts[0], velocities[0], '-', linewidth = 1)
    plt.xlabel('Tempo trascorso', fontsize = 10)
    plt.ylabel('Velocità', fontsize = 10)
	...
	'''
    plt.subplot(2, 1, 2)
    plt.plot([p[0] for p in people_pos[0]], [p[1] for p in people_pos[0]])
